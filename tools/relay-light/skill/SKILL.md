...
每个施工批次一个节点；coder 与 checker 批内同时在场（trigger 留空），scribe 等 coder done 后拉起，decider 仅在 blocked 时拉起；`close=agent:checker`（checker 通过才进下一批）。封口纪律：判定方判定 PASS 前，送审方与判定方均不记 `done`；FAIL 走 live 判定方的 `checkpoint` 路由回同一送审方；PASS 后按送审方→判定方顺序记终态。
...
11. **送审封口纪律**（W/C/X 适用，R 不适用）：判定方判定 PASS 前，送审方与判定方均不记 `done`；FAIL 走 live 判定方的 `checkpoint` 路由回同一送审方；PASS 后按送审方→判定方顺序记终态。送审信号 = `checkpoint` + `ready_for_review=<判定方>`（非终态，A145）；判定方 `done` 须带 `reviewed=<送审方实例>` 与 `ready_seq=<信号行号>`（A146）；轮次上限沿用 `limits.rework_max_rounds`，超限走 strategist → 用户裁决（A147）。
