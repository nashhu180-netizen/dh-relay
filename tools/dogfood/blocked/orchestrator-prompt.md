...
- 运行现场根：`{{RUN_ROOT}}`；run_id：`{{RUN_ID}}`。
- 提案工具：`{{TOOL}}`（PowerShell 脚本）。

做法：
1. 读两份输入，确认计划里只有任务 A（brief 在 `{{DOGFOOD}}/brief-A.md`）。
2. 把下面这份 JSON 原样写到 `{{RUN_ROOT}}/drafts/plan-v1.json`（目录不存在就建；`proposed_at` 填当前 UTC 时间 ISO-8601，如 `2026-08-15T12:00:00Z`；不要加 `plan_hash`，工具会回填；不要增删节点或改字段名）：
