...
//      没有任何机制指出「能力指纹变了」。F-047 的立论原话对 20 份 schema 逐字同样成立，
...
function frozenSchemas() {
  // **顶层 19 份已冻结协议 + 1 份共享定义模块**。DHR_61 增补 Attempt Receipt、pause、resolution 与 RPC v2 contracts，
  ...
  files.sort();
  if (files.length !== 19) {
    throw new Error(`contracts/ 顶层应有 19 份已冻结协议，实为 ${files.length} 份：${files.join(', ')}`
...
function build() {
  ...
  return {
    $comment:
      ...
      + ' **它同时是 20 份契约的 digest 基线**：改任一 schema 的任意一个字（含 description，本卡有意不剥离）'
...
if (argv.includes('--write')) {
  ...
  console.log(`已写 capability-baseline.json：${b.capability_manifest.protocols.length} 份（19 顶层协议 + 1 共享定义模块）`
...
if (errors.length === 0) {
  console.log(`capability 基线对证通过：${want.capability_manifest.protocols.length} 份（19 顶层协议 + 1 共享定义模块）digest 相符，capability_hash = ${got.capability_hash.slice(0, 16)}…`);
  process.exit(0);
}
