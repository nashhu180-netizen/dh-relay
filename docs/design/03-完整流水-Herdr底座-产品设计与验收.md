...
### H10 · 终端后端可配置，psmux 不变、herdr 追加（**用户 2026-08-17 拍板**）

> **用户原话口径**：「底座干脆改成可配置的。psmux 不变，再加上 herdr。」本条由「我建议的对冲手段」升级为**用户拍板的产品决定**，并因此改变了拆卡形态——见 [`dev_plan/P3`](../dev_plan/P3-可配置终端后端与Herdr底座-开发方案.md) §0：「后端可配置化」与「herdr 适配器」拆成两张卡，**前者的价值与 `H-A8-1` 结果解耦**，判否也不作废。
...
1. ~~`H6` 前置闸怎么跑~~ → **已定（2026-08-17）**：独立一份 [`dev_plan/P3-可配置终端后端与Herdr底座-开发方案.md`](../dev_plan/P3-可配置终端后端与Herdr底座-开发方案.md)（事件 `DHR-B-03`），三卡 `DHR_22`（后端可配置化）→ `DHR_23`（herdr 适配器）→ `DHR_24`（实战与判定）。**此前"spike 插队 P2"的拆法已被用户推翻并整体还原**，`P2-完整流水-开发方案.md` 逐字回到 `DHR-B-02` 状态。
2. ~~`U-A8-9` 专项验证由谁做~~ → **已做（2026-08-17，主会话亲跑）**：结论见 `E-A8-11` 与 §8.3。
3. **若 `H-A8-1` 通过，02 与 03 何时合并**：把 03 的差异面回灌进 02 形成单一输入（白名单只留一份），还是长期两份并列？——建议回灌成一份，避免两份输入漂移（本仓已有"两份入口逐渐漂移"的明确教训）。
4. **`H-A8-1` 判否时 `DHR_23` 产出怎么处置**（候选：保留为默认永不启用的后端选项 / 整体 revert / 降级为测试夹具）——已挂 `P3` §0 理解问题，**开工前不必答，`DHR_23` 收口前必须答**。
5. 是否安装 herdr 的 `claude`/`codex`/`kimi` 集成（`U-A8-2`）——它会写入用户 CLI 配置目录，属需要授权的动作。
