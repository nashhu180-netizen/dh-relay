...
## 4c. `DHR-B-03` 讲解与理解确认（独立 P3 计划，三卡）

- **事件**：`DHR-B-03`，`stage=B-new`，新建 [`dev_plan/P3-可配置终端后端与Herdr底座-开发方案.md`](../../dev_plan/P3-可配置终端后端与Herdr底座-开发方案.md)。
- **为什么必须现在排卡（而不是等 `design/03` 确认）**：`design/03` 的 P0 前置闸 `H-A8-1`（用户亲判 Herdr 交互体验）**只能靠真跑来判**，真跑需要 relay 有 herdr 适配器，而 relay 今天只有 psmux 适配器 → 不排卡就形成死锁（不做适配器 ⇒ 判不了闸 ⇒ 方案永远确认不了 ⇒ 永远不排适配器）。该例外已在 P3 §0 显式登记，未伪装成"依据已确认"。

### 起草经过的一次推翻（如实记录，不掩盖）

- **首版（2026-08-16）**：按用户点选的「乙 · spike 插队 P2」，把单卡 `DHR_22` 插进 `P2-完整流水-开发方案.md`（`stage=B-adjust`），并为此**显式豁免了 P2 §4 完工清单中「无非 psmux 后端」一条**。
- **推翻（2026-08-17，用户）**：「devplan 独立一份吧。底座干脆改成可配置的。psmux 不变，再加上 herdr。」
- **推翻带来的两处实质改善**：
  1. **豁免消失**。P2 §4 那条「不做非 psmux 后端」约束的是 P2 的十八张交付卡；herdr 工作独立成 P3 后，该条**原样成立**，不再需要硬拗的豁免。`P2-完整流水-开发方案.md` 已**整体还原到 `DHR-B-02` 状态、逐字未变**。
  2. **拆卡形态变了，且更好**。「后端可配置化」与「herdr 适配器」拆成两张卡后，`DHR_22` 的产出**与 herdr 去留解耦**——它只是把已存在的六动词适配器抽象接出一个显式选择点，psmux 仍是默认。这消掉了首版拆法最别扭的一处：整卡产出都押在 `H-A8-1` 上、判否就全废。现在**判否只影响 `DHR_23`/`DHR_24`**。
- **教训候选**：把"为了做决定而做的验证"硬塞进"交付计划"，会逼出对交付计划自身约束的豁免；**豁免自己刚定稿的完工清单，就是拆错地方的信号**——该独立成计划，而不是改约束去迁就。

### 三卡形态

| 卡 | 一句话 | 与 `H-A8-1` 的关系 |
|---|---|---|
| `DHR_22` | 终端后端从写死 psmux 改为显式可配置，**默认行为零变化** | **解耦**，判否也保留 |
| `DHR_23` | herdr 适配器（六动词 + 句柄纪律 + 启动 fail-closed） | 判否则按用户回答处置 |
| `DHR_24` | 真卡实战 + `J1～J5` 对照表 + 用户人判 | 就是判 `H-A8-1` 的那张卡 |

- **用户拍板汇总**：① `design/03` = 平行候选 A 方案，B 开工前做底座对比决策（08-16）；② 实战载体 = 接着跑 `DHR_04` 剩余棒（08-16）；③ **DevPlan 独立一份**（08-17，推翻 ①' 的插队方案）；④ **终端后端可配置：psmux 不变，追加 herdr**（08-17）。
- **理解问题**：`H-A8-1` 判否时，`DHR_23` 产出的 herdr 适配器怎么处置？
- **待用户回答**：（未答。候选=保留为注册表可选但默认永不启用的后端 / 整体 revert / 降级为测试夹具。**开工前不必答，`DHR_23` 收口前必须答**。）
