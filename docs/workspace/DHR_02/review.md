...
| 验什么 | 做什么 | 通过标准 | 结果 |
|--------|--------|----------|------|
| 机器证全绿 + 两轮复核收敛（E10 证据展示） | 查看 AI 在对话展示的 `run-relay-tests.ps1` 输出、两条回放事件签名、探针记录与两轮复核结论 | RELAY ALL PASS（11 套件）+ 0 open P0/P1 + 轮1/轮2 均已落账 | [x] 用户 2026-08-15 对话 AskUserQuestion 点选「认可，执行本地收口（推荐）」 |

---

- 确认记录：2026-08-15 用户在对话 AskUserQuestion 点选「认可，执行本地收口（推荐）」——授权包=squash 合入/集成复跑/verify/销户/删任务树；包外 push/DHR_03 未授权
- verify 提交 SHA：见 `git log --grep="^verify(dh-relay): DHR_02"`（squash 合入=f34e212）
- 签名：hyf（chat-confirm 代签）　　时间：2026-08-15 18:28
...
|---------|--------|----------------------|----------------------|-------------------------------|---------------|--------------------------------|
| 2026-08-15 18:28 | hyf | releasePacket-DHR_02 | shown-v1（E9 七段+E10 证据展示·对话） | E-010 亲跑 RELAY ALL PASS 405/0 + 三轮复核落账 + 三方探针 | DHR02-A1A2/A3/A4/A5/A6/FAKE | 通过 |
