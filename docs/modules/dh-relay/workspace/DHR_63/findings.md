...
| F-6302 | P1 | open（移交） | 运行时 `loadExecutorProfiles` 固定以 `resolveAlias:false` 调用 validator；完整 registry 的坏 alias 在 validator 中返回 `E_UNRESOLVED_ALIAS`，但 loader 仍接受，不能证明 P6-RI-A5 的“拒绝真实启动”。 | B-25 已移交 DHR_65（P6-RI-A5 runtime/mutation）；DHR_63 不改 loader/validator/生产代码，也不以 validator 负例冒充 runtime fail-closed。 |
