...
| 验什么 | 做什么 | 通过标准 | 结果 |
|---|---|---|---|
| DSH-off 与 Herdr 状态映射 | 读 E-3302~E-3306 账本及真实 smoke 记录 | 机器项可回链，受限项未冒充全达成 | [x] 2026-08-30 用户对话「DHR33：都接受」 |
| Linux/Oracle 差异与下游遗留 | 查看 E-3301/E-3307/E-3308 及 findings F-3/F-6/F-11 | 接受延后与 DHR_35 承接范围 | [x] 2026-08-30 用户对话「DHR33：都接受」 |

- [x] **E10 收口确认与 verify**（2026-08-30 用户对话「DHR33：都接受；裁决1：B」）：已查看证据，认可收口（注：squash `66dd16a` 已按 2026-08-29 委托先行合入 master，不认可可指令回滚）。做什么：读本表验收行 + review-consistency-opus.md §六/§八。
- [x] **E-3303 现场人验**（2026-08-30 用户对话「DHR33：都接受」）：真实 smoke 为一次性现场（DSH 未运行 + 真 Run 四命令 + 附着指令），复核只能核文本一致性。做什么：读 progress C 段，认可即勾。
- [x] **E-3304 受限受理**（2026-08-30 用户对话「DHR33：都接受」）：事件快路上游能力缺失（F-6）、能力 hash 未产出（F-11），均交 DHR_35。接受受限达成即勾。
- [x] **E-3307/E-3308 Oracle 差异受理**（2026-08-30 用户对话「DHR33：都接受」）：`awaiting_result` 语义替换、版本/能力 hash 降级（brief 完成条件 5②③，候选-42 机制）。
- [x] **P6-M6 延后受理**（2026-08-30 用户对话「B-22：确认延后；DHR33：都接受」）：接受 Linux 项延后至阶段闸裁决（B-22① 汇合点）。
- [x] **候选-40 模型身份裁决**（2026-08-30 用户对话「裁决1：B」）：换人复核冻结为 fresh 实例 + 独立会话；模型身份冲突继续如实并存登记，但不作为复核有效性的判据（见 review-lessons-opus.md §四-3）。
