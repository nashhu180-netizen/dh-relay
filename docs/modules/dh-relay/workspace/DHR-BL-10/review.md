...
|------|------------|----------|--------|-------|---------|-------------|---------|-----------|-----------|----------------|------------------|---------------------|
| zcode 真实交棒 | `result`/`checkpoint`/`handoff`/进程退出码与三源时间线 | human | H1 | 等价覆盖 | ③∈①⊂② 且身份四元组一致 | E-011、E-025~E-028；用户 2026-08-27 确认通过 | Windows · zcode 0.16.5 | OS 进程账 + 仓外会话元信息 + 入仓工件 | 不证明常驻交互位 | v1 | 用户可查看完整证据 | chat-confirm |

**业务化五段展示区**（人验项证据先走这五段；原始断言/完整日志退为可追溯附录，只写 `npm test ✅`/"我跑过了" 不算人验展示）：

- 要证明啥：zcode 能在现役 headless 位按 relay v1 协议真实交棒。
- 期望值：result 成功、身份四元组一致、checkpoint 有内容、进程 exit 0，三源时间线自洽。
- 实际值：E-011 与 E-025~E-028 满足上述条件，用户于 2026-08-27 查看后确认通过。
- 差没差：无阻断差异；大小写 fail-closed 的追加授权另见 RQ-1 记录。
- 证据局限：只覆盖 headless 一次性位，不覆盖常驻交互施工位。
