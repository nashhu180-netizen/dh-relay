...
| C-003 | `relay-core/runtime/service.mjs`、`workflow-driver.mjs`、Herdr completion 接点 | 找唯一 gate 并切断宿主观测/judge 直写结果支路。 |
...
| # | 改动文件 | 怎么改 | 怎么验 |
|---|---|---|---|
| 1 | Test · `relay-core/test/**` | 先写 Receipt mismatch、重复、缺失、v1/v2、prepared recovery 与宿主观测不直写 Result 的失败用例。 | `node --test ...` 先红，失败仅因 bridge 未实现。 |
...
| 4 | Modify · `workflow-driver.mjs`、`executors/herdr/**`、CLI | driver 只等待 submission；completion 仅传 submission；封堵 host/judge/capture 直写和 quota/fallback 支路。 | 负例进入人工等待，零 Result 写入。 |
