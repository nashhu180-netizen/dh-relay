...
| C-003 | `relay-core/profiles/validate-profiles.mjs`、`profiles.test.mjs` | 对齐 formal validator 行为，不修改既有 validator 或测试。 |
...
| 4 | Record · `workspace/DHR_65/**` | 记录红绿、mutation、卫生和需求对齐证据；不写 registry 正文、配置值或凭据。 | `git diff --check`、secret-shaped 扫描、`dh dh-relay` 均有终态。 |
