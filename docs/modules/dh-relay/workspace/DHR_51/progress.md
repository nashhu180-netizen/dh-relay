...
| 2026-08-22 | 历史验收记录回填（来源：verify `bd6b3b5`） | E-202：E11 对话内点选「确认收口，含真实终端演示」；该 verify 的 `Verified-Via` 与 Evidence 记录真实场景为关闭启动 shell 后新 shell 读数 `host=alive`，SIGKILL 后接管 epoch 递增。此条补齐 E-201 的人判闭环来源，不替代机器侧实录。 | E11 已确认 | 历史收口已完成 |
...
|---|---|---|---|---|
| E-201 | 真机 | 临时仓 `R001-demo-20260822`：启动 detached 宿主后关闭启动 shell；新 shell 执行 `status.mjs` 得 `host=alive`、events=2、租约续期；SIGKILL 后新会话接管 epoch=2；现场已清理。 | observed | 机器侧真实场景预演：关启动终端后宿主仍可读，强杀后可安全接管。 |
| E-202 | 人判 | 历史 verify `bd6b3b5` 的 `Verified-Via: chat-confirm E11 对话内 AskUserQuestion 点选「确认收口，含真实终端演示」`；同一不可变提交 Evidence 记载 E-201 场景的 `host=alive` 与接管 epoch 递增。 | observed | E11 的用户确认与真实终端演示闭环；补齐 E-201 的人判来源，不替代机器侧实录。 |
