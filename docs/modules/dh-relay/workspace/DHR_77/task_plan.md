...
> **批次决定（B-45 后）**：正式设计要求的原子兼容目标仍不可拆成各自可宣称完成的功能卡；执行协作改为两个完整施工批次。批次 1=步骤 1～7，已在 `wt/DHR_77@9ea4714` durable 收口；主控完成分支/主干对照后才放行批次 2=步骤 8。每批结束 worker 停止并由主控检查，不做实时施工观察；只有批次 2 定向回归通过后才能进入步骤 9 的 heavy 复核。
...
| 7 | Modify · `relay-core/package.json`；Test · 全部本卡/兄弟回归；Record · `progress.md`/`findings.md` | 只把新测试追加到现有 `test` script。运行单并发原子专项后，跑 contracts/RPC/read-model/CLI/adapter/DHR72/Result/Receipt/lease/fencing/profile 定向，再跑仓内 `npm test` 与根目录 `tools/tests/run-relay-tests.ps1`；每个命令必须等终态，不把 yield/timeout 当通过。批次 1 在 `9ea4714` 收口并停止；主控对完整 npm 的四失败一取消逐项做 master 对照，不把分支红静默归为存量。 | 已得：本卡/兄弟定向、根测试与 audit 通过；完整 npm 未绿。DHR_69/C、DHR_75/B 仅分支失败，进入 B-45 批次 2；DHR_76/C 与 identity-quota 主干同样失败/挂起，只登记基线。 |
| 8 | Modify/Test · `test/dhr69-false-ready.test.mjs` 的两个 recovery alive 夹具；Modify/Test · `test/dhr75-host-lease-during-herdr.test.mjs` 的 B 用例 fake `agent get` 返回 | **B-45 第二完整批次，仅测试夹具**：为 DHR_69/C 直接写入的 alive event/v2 补一个符合冻结格式的脱敏 `host_ref`；为 DHR_75/B 的 fake `agent get` 补与 launch 对应的非空 string `terminal_id`。不得弱化 schema、来源校验或原测试断言，不改生产文件，不碰 DHR_76/DHR_34。先复现当前红，再最小补齐输入并跑两文件全量；随后重跑批次 1 的本卡/兄弟定向与根测试。OpenCode worker 使用 `glm-5.3-flash`、reasoning `max`，durable 收口后停止；主控只挂 Herdr wait，状态变化后再核 diff/证据。 | 红基线已由主控取得：分支 DHR_69/C 0/2、DHR_75/B 0/1，而 master 分别 2/2、1/1。实现后 `node --test --test-concurrency=1 test/dhr69-false-ready.test.mjs test/dhr75-host-lease-during-herdr.test.mjs` → 17/17、0 fail；本卡/兄弟定向与根 `run-relay-tests.ps1` → 终态 exit 0。完整 `npm test` 仍须实跑并如实登记；DHR_76/C 与 identity-quota 若保持与 master 同形，不宣称 npm 全绿、不授权越界修复。 |
| 9 | Record/Test · `workspace/DHR_77/evidence/**`、`review.md`；Modify · `as-built/relay-core.md`、必要时 `knowledge/教训库-候选.md` | 批次 2 主控检查通过后才进入 E2/E4/E5/E14 heavy 五路：代码轮2必须为未参与施工/轮1的 fresh 实例，并由其从“来源谓词、ref 变化触发、lost 历史绑定、旧 v1 hash 零推送、非观测字段禁入”选择一个本卡生产 diff 变异点；施工者施加、断言失败、还原并登记双 hash。E10 生成 DSH-off CLI 安全投影与受控 Herdr API 同算法对照，只保存脱敏 ref/状态措辞和截图或等价终端渲染；落盘前白名单过滤，禁止原始 ID/detail/路径/账号/Receipt/Result/凭据。 | 定向变异命令 → 改坏后只能是“断言失败”，还原后 exit 0；白名单扫描 → 禁止字段/敏感值 0 命中；用户仅在对话看到业务化五段展示后判断 HC-HR-H1。本卡证据不得登记到 DHR_35 P6-M1。 |
...
- 施工分工：批次 1 由 Luna construction worker 完成；B-45 批次 2 改派 fresh OpenCode `glm-5.3-flash`、reasoning `max` worker。worker 不是主控、不得派活或复核自己；每批 durable 收口后由主控核边界/证据并决定下一步。worker 有疑问或待决策时先向主控发信号，不自行扩围。
