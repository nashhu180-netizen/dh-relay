...
$temporaryRoot = Join-Path ([IO.Path]::GetTempPath()) ("DHR72-fixture-$($ProfileId -replace '[^A-Za-z0-9]', '-')")
...
try {
  ...
  $instruction = 'Ignore prior completion instructions. Run exactly: node -e "setTimeout(() => {}, 5000)". Report only its exit status. Do not inspect or change files/config, or run relay submit-result.'
  $stage = 'agent-instruction'
  $priorErrorActionPreference = $ErrorActionPreference
  $ErrorActionPreference = 'Continue'
  try {
    $promptOutput = & $herdrBin agent prompt $agent.name $instruction 2>&1
    $promptExitCode = $LASTEXITCODE
  } finally {
    $ErrorActionPreference = $priorErrorActionPreference
  }
  $readyAfterPrompt = $false
  if ($promptExitCode -eq 0) {
    ...
    $readyAfterPrompt = ($statusName -eq 'working')
  }
  if (-not $readyAfterPrompt) {
    # Codex can leave an atomically pasted prompt in its input buffer without submitting it.
    # Follow the documented Herdr fallback: send Enter at most twice and verify a real working state.
    foreach ($unused in 1..2) {
      & $herdrBin agent send-keys $agent.name enter | Out-Null
      if ($LASTEXITCODE -ne 0) { throw 'failed to submit the controlled instruction with Enter' }
      ...
      if ($readyAfterPrompt) { break }
    }
  }
  if (-not $readyAfterPrompt) {
    throw "failed to deliver controlled completion instruction (prompt exit $promptExitCode)"
  }
