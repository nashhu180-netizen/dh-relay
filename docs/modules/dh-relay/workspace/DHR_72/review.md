...
|---|---|---|---|---|---|---|---|---|
| `relay-core/runtime/workflow-driver.mjs:389-400` | idle/done 分支持续轮询 → 写完 missing-Result Attention 后插入 `return` | 首次 idle 提前停止 | `initial idle remains observable until later working records a checkpoint` | `node --test --test-concurrency=1 --test-timeout=60000 --test-reporter=spec --test-name-pattern="initial idle remains observable until later working records a checkpoint" test/dhr72-continuous-observation.test.mjs` | `sha256:61EA3FFD71E88C9E93ED46E10ECA1786CE41E9912AD296206C41B5279DBD1030` | `git-blob:505ff9454863bd319429b33c27e5ca2d3cf7ae69` | `dhr72_code2_0905` | exit=1，`timeout:working checkpoint`；还原后 1/1、exit=0，`git diff --exit-code`=0。 |
...
| 比例守卫与 DHR_74 回签 | 守卫变异断言失败 + master R31 | machine | DHR72-H | 部分 | 见 brief #6 | 守卫已红→绿；DHR_74 R31 未清 | master after merge | dh check | DHR_74 结论不重判；当前授权不含 merge | v1 | test | user-D-start-E10 |

→ 当前状态：**E11 已获用户明文认可；正按 releasePacket-DHR72-v1 执行本地 squash、主干回归与机器证 H，尚未 verify/收口。**
...
- **未满足项 / 硬停点**：机器证 H 必须在 DHR_72 候选入 master 后才可回填 DHR_74 R31。当前 `dh dh-relay` 为 5 failure：DHR_75 R18/R30/R31、DHR_74 R30/R31；不得把 H 的预期效果写成已发生，也不得承诺 verify 必过。
