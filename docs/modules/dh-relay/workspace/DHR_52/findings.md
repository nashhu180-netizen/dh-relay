...
| F-225 | P1 | E12 主干全量复验的既有 DHR_51 `lost_lease` 用例失败：测试覆写 lease 后，宿主仍在初始 `lease_acquired` 写账，writeGuard 抛 `E_LEASE_HELD:lease-lost` 而未进入 lost-lease 收敛分支。 | E-078；`relay-core/test/runtime.test.mjs:528`；`relay-core/runtime/host.mjs:61` | 用户授权独立维护树 `DHR_51-F225`；初始化与 tick 期仅对精确 `E_LEASE_HELD:lease-lost` 同义收敛，其他错误仍抛。两轮复核收敛后 squash `e55a7f8`；E-079 主干全量 90/90 + 34/34 + audit/manifest/baseline/diff 全绿。 | resolved |
| F-226 | P1 | `verify(dh-relay)` 的强制提交钩子要求完整 `dh dh-relay` 零失败；E-080 实测仍有 34 条历史失败（design 输入结构、DHR_03/29/49/51），因此拒绝产生 verify 提交。 | E-080；提交钩子完整输出 | 不绕过钩子、不在 DHR_52 顺手修跨卡历史债。需用户单独授权治理修复范围后才可清零并重试 verify；在此之前保留 DHR_52/DHR_51-F225 worktree。 | open-blocking |
