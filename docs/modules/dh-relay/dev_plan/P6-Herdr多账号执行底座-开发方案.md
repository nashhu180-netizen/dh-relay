...
下一步（2026-09-05 DHR_72 H 实测后）: **当前硬阻塞为 scanner 仍要求 DHR_74 单卡自有变异表，未将已入 `master@13c072d` 的 DHR_72 H 证据传递给 DHR_74**。在该机制缺口未获用户另行裁决前，不补造 DHR_74 表、不改状态绕闸、不生成 verify；保留 DHR_72/DHR_74 任务树。本次授权仍不含 push、deploy、环境/生产写入、DHR_73 或 DHR_35。
...
| DHR_72 | 修 driver 单次 idle 采样即永久退出观测 | 标准 | 待验收 | DHR_71（已完成）、DHR_75（待验收） | [workspace/DHR_72/](../workspace/DHR_72/) | squash `13c072d`；verify 未生成 | 任务类型=heavy；用户 2026-09-05 E11 明文认可；主干专属 8/8、poll 1/1、冻结 55/55 全绿，真实 F 保持有效；本卡 R18/R31 已清。H 守卫/变异已入主干，但 scanner 仍报 DHR_74 `mutation-table-missing`，未实现跨卡传递；本卡 verify 因实时强闸不绿未生成，任务树保留；不含 push/deploy/下一卡 |
