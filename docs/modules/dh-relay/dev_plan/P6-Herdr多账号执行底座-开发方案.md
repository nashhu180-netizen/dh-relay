...
现状: DHR_32、DHR_33、DHR_61、DHR_34、DHR_63、DHR_64、DHR_65、DHR_66、DHR_67、DHR_68、DHR_69、DHR_70、DHR_71、DHR_75、DHR_76 已完成；DHR_74 待验收；**DHR_76（完整 Profile 校验非阻塞化，heavy）已于 2026-09-05 经用户认可本地收口，squash `23de8cf`、verify `6adb54e`，`release_mode=full`；DHR_75 已于同日经用户 E11 确认，squash `24ba064`、verify 见本提交；DHR_72 进行中，须 rebase 已含 DHR_75 的 master 后重跑自身证据；DHR_35 进行中且 `blocked-by:DHR_72`**。P6-M1 仍未达成
...
下一步（2026-09-05 DHR_75 收口后）: **① DHR_72 rebase 已含 DHR_75 的 master，重跑专属套件、冻结回归与自己的真实机器证 F → ② DHR_72 机器证 H 入 master 后使 DHR_74 的 R31 清零，回填并补签 DHR_74 verify → ③ 按既有边界推进 DHR_73 调查；DHR_35 在 DHR_72 收口后由用户重新放行**。这里只登记后续顺序，不构成任何下一卡的新施工授权；各卡实录只证明自己的 baseline
...
阻塞: **DHR_72 的 `blocked-by:DHR_75` 已随 DHR_75 收口解除，下一节点是 rebase 后的 DHR_72 专属证据；DHR_35 仍 `blocked-by:DHR_72`（B-35）**。DHR_72/DHR_35 后续仍须逐卡独立取证。DHR_74 R31 只能在 DHR_72 机器证 H 入 master 后回填清零。F-3516/F-3519 的其它启动停摆机制继续由 DHR_73 调查。Linux SSH 真实 smoke 按 B-22 延后，P6-M6 只能在阶段闸记延后/受限
-->
...
| DHR_75 | 让 Herdr CLI 保持有界失败但不阻塞 Host lease 续租 | 标准 | 已完成 | DHR_70、DHR_76（均已完成） | [workspace/DHR_75/](../workspace/DHR_75/) | squash `24ba064`；verify 本提交（2026-09-05） | 任务类型=heavy；用户 E11 明文确认；新基线专项 7/7、adapter 24 pass/3 frozen skip/0 fail、真实 F 的首条 alive observation 早于 lease expiry，fresh 需求复核 PASS；主干本轮整套无终态不记绿，D 定向两项 1/1；模块体检余 DHR_74 R31，不含 push/deploy/下一卡 |
...
| DHR_72 | 修 driver 单次 idle 采样即永久退出观测 | 标准 | 进行中 | DHR_71、DHR_75（均已完成） | [workspace/DHR_72/](../workspace/DHR_72/) | `wt/DHR_72@bfc6e72` | 任务类型=heavy；此前用户已 D-start；DHR_75 收口已解除依赖，在途分支保留不回退；下一节点是在已含 DHR_75 的 master rebase 后，重跑专属套件/冻结回归/真实机器证 F。长期 idle、五出口与 `workflow-driver.mjs` 主体归属不变；DHR_76 对该文件仅限 loader await 后的 stopping 复查；本次状态同步不是新施工授权 |
