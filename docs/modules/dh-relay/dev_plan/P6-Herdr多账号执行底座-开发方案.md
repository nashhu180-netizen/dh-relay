...
现状: DHR_32、DHR_33、DHR_61、DHR_34、DHR_63、DHR_64、DHR_65、DHR_66、DHR_67、DHR_68、DHR_69、DHR_70、DHR_71、DHR_76 已完成；DHR_74 待验收；**DHR_76（完整 Profile 校验非阻塞化，heavy）已于 2026-09-05 经用户认可本地收口，squash `23de8cf`、verify `6adb54e`，`release_mode=full`；DHR_75 已完成施工/复核，原 `blocked-by:DHR_76` 已解除，下一步需在新 master 上独立 rebase、重跑专项/回归及自己的真实 F；DHR_72 进行中且 `blocked-by:DHR_75`；DHR_35 进行中且 `blocked-by:DHR_72`**。P6-M1 仍未达成
...
下一步（2026-09-05 DHR_76 收口后）: **① DHR_75 在已含 DHR_76 的 master 上独立 rebase，重跑专项、回归与自己的真实 F，恢复至 E10 → ② DHR_72 rebase 已含 DHR_75 的 master，重跑专属套件、冻结回归与自己的真实机器证 F → ③ DHR_72 机器证 H 入 master 后使 DHR_74 的 R31 清零，回填并补签 DHR_74 verify → ④ 按既有边界推进 DHR_73 调查；DHR_35 在 DHR_72 收口后由用户重新放行**。这里只登记后续顺序，不构成 DHR_75 或任何下一卡的新施工授权；各卡实录只证明自己的 baseline
...
阻塞: **DHR_75 的 `blocked-by:DHR_76` 已随 DHR_76 收口解除；DHR_72 仍 `blocked-by:DHR_75`（B-40）；DHR_35 仍 `blocked-by:DHR_72`（B-35）**。DHR_75 尚须在新基线独立重跑并取得自己的真实 F，当前状态不等于自动开工或完成。DHR_75/DHR_72/DHR_35 后续仍须逐卡独立取证。F-3516/F-3519 的其它启动停摆机制继续由 DHR_73 调查。Linux SSH 真实 smoke 按 B-22 延后，P6-M6 只能在阶段闸记延后/受限
-->
...
| DHR_75 | 让 Herdr CLI 保持有界失败但不阻塞 Host lease 续租 | 标准 | 进行中 | DHR_70、DHR_76（均已完成） | [workspace/DHR_75/](../workspace/DHR_75/) | `wt/DHR_75@44e8efc` | 任务类型=heavy；施工、专项、有效变异和五路复核已闭合；原 `blocked-by:DHR_76` 已解除，须在新master上独立rebase并重跑专项、回归与自己的真实F后再恢复至E10；本次未启动下一节点，不含E11/verify/合入/push/部署 |
...
| DHR_72 | 修 driver 单次 idle 采样即永久退出观测 | 标准 | 进行中 | DHR_71（已完成）、DHR_75（进行中） | [workspace/DHR_72/](../workspace/DHR_72/) | `wt/DHR_72@bfc6e72` | 任务类型=heavy；此前用户已 D-start；现 **`blocked-by:DHR_75`**（B-40/B-41），在途分支保留不回退；DHR_75 合入后再 rebase，重跑专属套件/冻结回归/真实机器证 F。长期 idle、五出口与 `workflow-driver.mjs` 主体归属不变；DHR_76 对该文件仅限 loader await 后的 stopping 复查；本次状态同步不是新施工授权 |
...
- [ ] DHR_32~35、DHR_61、DHR_63、DHR_64、DHR_65、DHR_66、DHR_67、DHR_68、DHR_69、DHR_70、DHR_71、DHR_72、DHR_73、DHR_75、DHR_76 全部销户。
