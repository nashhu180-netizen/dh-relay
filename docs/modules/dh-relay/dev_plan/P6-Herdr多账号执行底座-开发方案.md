...
现状: DHR_32、DHR_33、DHR_61、DHR_34、DHR_63、DHR_64、DHR_65、DHR_66、DHR_67、DHR_68、DHR_69、DHR_70、DHR_71 已完成；DHR_74 待验收；**`DHR-B-41`（2026-09-05 已确认）新增 DHR_76（完整 Profile 校验非阻塞化，heavy，进行中）；DHR_75 已完成施工/复核但真实 F 被启动前约 19.7 秒同步 Profile 校验击穿 15 秒 lease，现 `blocked-by:DHR_76`、停在 E10 前；DHR_72 进行中且 `blocked-by:DHR_75`；DHR_35 进行中且 `blocked-by:DHR_72`**。P6-M1 仍未达成
...
<!-- dh:planning-event:v1 id=DHR-B-41 stage=B-adjust artifact=dev_plan/P6-Herdr多账号执行底座-开发方案.md review=../design/evidence/38-DHR76-Profile同步校验阻塞HostLease-B调整交叉审核记录.md#review-b41 understanding=../design/evidence/38-DHR76-Profile同步校验阻塞HostLease-B调整交叉审核记录.md#understanding-b41 -->
...
| DHR_75 | 让 Herdr CLI 保持有界失败但不阻塞 Host lease 续租 | 标准 | 进行中 | DHR_70（已完成）、DHR_76（未开始） | [workspace/DHR_75/](../workspace/DHR_75/) | `wt/DHR_75@44e8efc` | 任务类型=heavy；施工、专项、有效变异和五路复核已闭合，但真实 F 在 Attempt 前被约 19.7 秒同步 Profile 校验击穿 15 秒 lease，现 **`blocked-by:DHR_76`、停在 E10 前**；DHR_76 合入后 rebase 并自跑新基线真实 F；不含 E11/verify/合入/push/部署 |
| DHR_76 | 让完整 Profile registry 校验 fail-closed 且不阻塞 Host lease | 标准 | 进行中 | DHR_65、DHR_70（均已完成） | [workspace/DHR_76/](../workspace/DHR_76/) | | 任务类型=heavy；B-41 新增；全表 alias/config 校验仍在 Attempt 前整体 fail-closed，默认 TTL/fencing 不变；异步返回后仅极窄复查既有 stopping；不改用户级 registry、Herdr CLI、lease/host/Store/contracts；须另行 D-start |
