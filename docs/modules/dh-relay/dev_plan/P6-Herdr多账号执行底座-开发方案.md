...
下一步（2026-09-05 DHR_72 E11）: **① 按已认可的 releasePacket-DHR72-v1 执行本地 squash/合入 master → ② 主干回归后执行机器证 H 并只按真实结果回填 DHR_74；DHR_74 verify 与 DHR_72 verify 均不得绕过实时全仓闸**。本次授权不含 push、deploy、环境/生产写入、DHR_73 或 DHR_35；强闸不绿时保留任务树与未 verify 状态。
...
| DHR_72 | 修 driver 单次 idle 采样即永久退出观测 | 标准 | 进行中 | DHR_71（已完成）、DHR_75（待验收） | [workspace/DHR_72/](../workspace/DHR_72/) | `wt/DHR_72@bfc6e72` | 任务类型=heavy；此前用户已 D-start；DHR_75 的实现已入主干但仍待 verify，在途分支保留不回退；DHR_72 是清零 DHR_74 R31 的唯一已登记路径，须在已含 DHR_75 的 master rebase 后重跑专属套件/冻结回归/真实机器证 F。长期 idle、五出口与 `workflow-driver.mjs` 主体归属不变；DHR_76 对该文件仅限 loader await 后的 stopping 复查；本次状态同步不是新施工授权 |
