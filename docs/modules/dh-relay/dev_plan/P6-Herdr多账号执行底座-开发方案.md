...
现状: DHR_32、DHR_33、DHR_61、DHR_34、DHR_63、DHR_64、DHR_65、DHR_66、DHR_67、DHR_68、DHR_69、DHR_70、DHR_71 已完成；DHR_74 待验收；**`DHR-B-40`（2026-09-04 已确认）新增 DHR_75（Herdr 异步有界调用与 Host lease 续租，heavy，未开始）；DHR_72 已按此前 D-start 进行中，现 `blocked-by:DHR_75`；DHR_35 进行中，仍 `blocked-by:DHR_72`**。DHR_72 三次 DSH-off Codex 实录均在首条 Host Observation 前失租，P6-M1 仍未达成
...
下一步（2026-09-04 改写）: **① DHR_75 另行 D-start，修 Herdr CLI 同步阻塞并完成 heavy 五路复核/E10/E11 → ② DHR_72 rebase 已含 DHR_75 的 master，重跑专属套件、冻结回归与自己的真实机器证 F → ③ DHR_72 机器证 H 入 master 后使 DHR_74 的 R31 清零，回填并补签 DHR_74 verify → ④ 按既有边界推进 DHR_73 调查；DHR_35 在 DHR_72 收口后由用户重新放行**。DHR_75/DHR_72 的实录都不能替代 DHR_35 自跑两条真实闭环；本次 B-adjust 不构成 DHR_75 D-start
...
| DHR_75 | 让 Herdr CLI 保持有界失败但不阻塞 Host lease 续租 | 标准 | 未开始 | DHR_70（已完成） | [workspace/DHR_75/](../workspace/DHR_75/) | | 任务类型=heavy；B-40 新增；默认 TTL 仍为 15 秒，不放宽单写者/epoch fencing；只改 Herdr CLI/adapter 异步边界，不改 workflow driver、host、lease、service、Store 或 contracts；真实实录只证明首条 Host Observation 前 lease 新鲜，不替代 DHR_72 机器证 F / DHR_35 P6-M1；须另行 D-start |
