...
现状: DHR_32、DHR_33、DHR_61、DHR_34、DHR_63、DHR_64、DHR_65、DHR_66、DHR_67、DHR_68、DHR_69、DHR_70、DHR_71、DHR_76 已完成；DHR_74 待验收；**DHR_76（完整 Profile 校验非阻塞化，heavy）已于 2026-09-05 经用户认可本地收口，squash `23de8cf`、verify `6adb54e`，`release_mode=full`；DHR_75 已于同日经用户 E11 确认、squash `24ba064` 已入主干，但 `verify(dh-relay)` 被 DHR_74 R31 阻断，仍属待验收；DHR_72 已于 2026-09-05 经用户 E11 明文认可并 squash 入 `master@13c072d`，主干专属 8/8、poll 1/1、冻结 55/55 全绿；但清洁 master 上 scanner 因无任务分叉只对账未提交 diff，对 DHR_72 报 `anchor-outside-diff`，且仍对 DHR_74 报 `mutation-table-missing`，故 DHR_72 仍待验收、未 verify 且任务树保留；DHR_35 进行中且 `blocked-by:DHR_72`**。P6-M1 仍未达成
...
下一步（2026-09-05 DHR_72 H 实测后）: **当前有两个 scanner 机制阻塞：①清洁 master 无任务分叉时，DHR_72 的生产变异锚点被判 `anchor-outside-diff`；② DHR_74 仍被要求单卡自有变异表，未消费已入 `master@13c072d` 的 DHR_72 H 证据**。在该机制缺口未获用户另行裁决前，不补造 DHR_74 表、不改 DHR_72/DHR_74 状态绕闸、不生成 verify；保留两张任务树。本次授权仍不含 push、deploy、环境/生产写入、DHR_73 或 DHR_35。
...
阻塞: **DHR_72 的实现、专属/冻结回归、真实 F 与自动复核已完成 E10 备料；当前唯一卡内未满足项是机器证 H 的 master 回签，它按合同只能在 DHR_72 候选获用户授权并入 master 后执行。DHR_35 仍 `blocked-by:DHR_72`（B-35）**。实时 `dh` 另见 E-7222：master 还出现 DHR_75 R18/R31，不能按旧交接的“仅 DHR_74 R31”假定绿闸。DHR_72/DHR_35 后续仍须逐卡独立取证。F-3516/F-3519 的其它启动停摆机制继续由 DHR_73 调查。Linux SSH 真实 smoke 按 B-22 延后，P6-M6 只能在阶段闸记延后/受限
-->
...
| DHR_72 | 修 driver 单次 idle 采样即永久退出观测 | 标准 | 待验收 | DHR_71（已完成）、DHR_75（待验收） | [workspace/DHR_72/](../workspace/DHR_72/) | squash `13c072d`；verify 未生成 | 任务类型=heavy；用户 2026-09-05 E11 明文认可；主干专属 8/8、poll 1/1、冻结 55/55 全绿，真实 F 保持有效。H 守卫/变异已入主干，但清洁 master 上 DHR_72 R31=`anchor-outside-diff`、DHR_74 R31=`mutation-table-missing`；本卡 verify 因实时强闸不绿未生成，任务树保留；不含 push/deploy/下一卡 |
