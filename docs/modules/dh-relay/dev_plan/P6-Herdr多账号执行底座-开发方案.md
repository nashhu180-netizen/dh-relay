...
现状: DHR_32/33/34/61/63~72/74/76 已完成；DHR_75 待验收；DHR_73 未开始；**DHR_77（heavy）两批施工与代码轮 1 已收敛；heavy Review Batch 发现旧 alive event/v2 会被新 schema 当损坏、CLI 无法显示 `legacy 未提供`，且 capability 正式说明仍写旧 v1 hash。用户已选择 B-46 方案 A，接受精确形状级只读兼容例外，并指定右侧 Herdr 重新调用 Luna max 续做；DHR_35 仍 `blocked-by:DHR_77`**。P6-M1 仍未达成
...
下一步（2026-09-06）: 落盘 B-46 后把 `wt/DHR_77` rebase 到最新 master，右侧 Herdr 新启 Codex `gpt-5.6-luna`、reasoning `max` 执行 legacy 双读/CLI/文档/测试/纠错账整改；durable 收口后由原 Review Batch reviewer 窄复核，再做最终生产变异与 E9/E10。主会话只挂 Herdr wait，状态变化后进入主控检查点。授权不含 DHR_76/DHR_34 修复、完整 npm 债务扩围、真实产品 Agent、DHR_35、verify、合并、push 或 deploy。
...
<!-- dh:planning-event:v1 id=DHR-B-46 stage=B-adjust artifact=dev_plan/P6-Herdr多账号执行底座-开发方案.md review=../design/evidence/45-DHR77-legacy账本兼容例外-B调整交叉审核记录.md#review-b46 understanding=../design/evidence/45-DHR77-legacy账本兼容例外-B调整交叉审核记录.md#understanding-b46 -->
...
| DHR_77 | 原子实现 Herdr terminal-instance `host_ref`、RPC hash 与 DSH-off 安全展示 | 标准 | 进行中 | DHR_72（已完成） | [workspace/DHR_77/](../workspace/DHR_77/) | | 任务类型=heavy；B-43 新增；2026-09-06 用户明文 D-start；首批施工停在 `9ea4714` 主控检查点，B-45 经用户同意只扩 DHR_69/C、DHR_75/B 两条兼容夹具并续做第二批；单卡承接 `design/14` `HC-HR-A1..A5/H1`，证据不得替代 DHR_35 P6-M1；不含真实产品 Agent、DHR_35、verify/合并/push/deploy |
