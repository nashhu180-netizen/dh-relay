...
现状: DHR_32、DHR_33、DHR_61、DHR_34、DHR_63、DHR_64、DHR_65、DHR_66、DHR_67、DHR_68、DHR_69、DHR_70、DHR_71 已完成；DHR_74 待验收；**`DHR-B-40`（2026-09-04 已确认）新增 DHR_75（Herdr 异步有界调用与 Host lease 续租，heavy，已于同日 D-start、进行中）；DHR_72 已按此前 D-start 进行中，现 `blocked-by:DHR_75`；DHR_35 进行中，仍 `blocked-by:DHR_72`**。DHR_72 三次 DSH-off Codex 实录均在首条 Host Observation 前失租，P6-M1 仍未达成
...
下一步（2026-09-04 D-start 后）: **① DHR_75 在独立 worktree 施工并自动推进 heavy 五路复核至 E10，E11 等用户确认 → ② DHR_72 rebase 已含 DHR_75 的 master，重跑专属套件、冻结回归与自己的真实机器证 F → ③ DHR_72 机器证 H 入 master 后使 DHR_74 的 R31 清零，回填并补签 DHR_74 verify → ④ 按既有边界推进 DHR_73 调查；DHR_35 在 DHR_72 收口后由用户重新放行**。DHR_75/DHR_72 的实录都不能替代 DHR_35 自跑两条真实闭环；本次 D-start 不含 E11/verify/合入/push/部署
...
阻塞: **DHR_72 现 `blocked-by:DHR_75`（B-40，2026-09-04）；DHR_35 仍 `blocked-by:DHR_72`（B-35，2026-09-02）**。DHR_72 三次 DSH-off Codex 实录显示 15 秒 Host lease 在首条 Host Observation 前到期；`spawnSync` 饿死同事件循环续租是最强候选机制，但不是已排他的根因。DHR_75 只修该前置并守住 fencing；DHR_72 仍须独立证明持续观测/checkpoint，DHR_35 仍须自跑两条真实闭环。F-3516/F-3519 的其它启动停摆机制继续由 DHR_73 调查。Linux SSH 真实 smoke 按 B-22 延后，P6-M6 只能在阶段闸记延后/受限
-->

<!-- dh:planning-event:v1 id=DHR-B-40 stage=B-adjust artifact=dev_plan/P6-Herdr多账号执行底座-开发方案.md review=../design/evidence/37-DHR75-Herdr异步调用与HostLease续租-B调整交叉审核记录.md#review-b40 understanding=../design/evidence/37-DHR75-Herdr异步调用与HostLease续租-B调整交叉审核记录.md#understanding-b40 -->
...
| DHR_75 | 让 Herdr CLI 保持有界失败但不阻塞 Host lease 续租 | 标准 | 进行中 | DHR_70（已完成） | [workspace/DHR_75/](../workspace/DHR_75/) | `wt/DHR_75@14008bd` | 任务类型=heavy；2026-09-04 用户明文 D-start，授权独立 worktree 施工与自动收口至 E10；默认 TTL 仍为 15 秒，不放宽单写者/epoch fencing；只改 Herdr CLI/adapter 异步边界，不改 workflow driver、host、lease、service、Store 或 contracts；真实实录只证明首条 Host Observation 前 lease 新鲜，不替代 DHR_72 机器证 F / DHR_35 P6-M1；不含 E11/verify/合入/push/部署 |
...
| DHR_72 | 修 driver 单次 idle 采样即永久退出观测 | 标准 | 进行中 | DHR_71（已完成）、DHR_75（未开始） | [workspace/DHR_72/](../workspace/DHR_72/) | `wt/DHR_72@bfc6e72` | 任务类型=heavy；此前用户已 D-start；现 **`blocked-by:DHR_75`**（B-40），在途分支相对 `master@bbeffc4` 有 6 笔本卡提交，保留不回退；DHR_75 合入后 rebase 并重跑专属套件/冻结回归/真实机器证 F。仍承接 F-3517 与 F-3520 语义部分；长期 idle、五出口与 `workflow-driver.mjs` 归属不变；本次状态同步不是新施工授权 |
