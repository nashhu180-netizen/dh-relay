...
进行到: P4 ▸ CLI 与桌面控制面轨均已收口；P4 主报告正文保持 DHR_27 历史记录，DHR_50 附录已补对证、诚实差额、§4 归属二分和用户人判
下一步: P5 依既定阶段闸推进；DHR_30 的 DSH Bridge 条件部分与 DHR_31 的 DSH 附加客户端项已可按各自范围执行，终端一键跳转作为后续受控 Bridge / Runtime 能力方向
...
- **B-10 白名单例外（经用户明文批准，2026-08-18）**：列表投影 `relay.pilot-run-list/v1` 是 DHR_25 施工中由用户逐条授权新增的，`design/README.md`「拆计划依据」白名单正文（design/01、02、05、06）**没有它的字段级定义**。主会话向用户说明缺口与两条路径（(a) 批例外推进 / (b) 先 A′ 再重走 B）后，**用户选择 (a) 并批准本次例外**，口径如下：
  ...
  **例外已关闭（DHR_30 机械回注，2026-08-28）**：字段级定义已落 [design/06 §14](../design/06-多控制面与Headless-SSH运行-设计补充.md#14-read-model-字段级定义dhr_30-冻结)，可 grep 标记 `whitelist-exception-closed: DHR_30` 双端命中（本行 + design/06 §14）。本条例外口径自此仅作历史留痕。
...
- **验收口径**：
  ...
  - **人判 · 列表屏中途闸**（B-10 理解问答产出，用户 2026-08-18 选定）：列表屏首次渲染成功即**暂停扩张**，把**可复跑的操作路径**交用户，由用户**自己跑起来实际用一下**再对体验表态（不以截图代替亲跑；截图只作同步留痕）。用户若判「不如 CLI 好用」，主会话**必须把评价拆成两类、不得混谈**——
    ...
    > **【B-13 / 用户 2026-08-21 决定】三分桶分类法作废，改记原话。**
    >
    > 中途闸实跑时，主会话按上述规则 2 逐条给出候选归属表，用户在四道题上**全部弃桶、以自己的话作答**（原话见 `workspace/DHR_49/findings.md`「中途闸点选结果」）。用户判词是**直接结论**（「不应该放设置里」「后来改进放的位置可以」「本质是我增加需求了」），而非对成因的归类；三分桶要求用户先替主会话完成一次成因分析，实际用不上。
    >
    > **据此改口径**：本闸的人判验收项**不再要求用户逐条点选 (i)/(ii)/(iii)**。改为主会话（a）**原样登记用户判词**，（b）逐条核对该判词的**工程影响面**（是否动 Read Model 契约 / 是否动分堆键 / 是否越出本卡变更范围），（c）核对结论交用户确认。上文 (i)/(ii)/(iii) 三桶**保留为主会话自查用的分析框架**，不再是用户必答项；规则 2「未经用户逐条点选不得视为已拆完」**作废**。
    >
    > 规则 1（禁揉一句）、规则 3（止损优先）**继续有效**——止损判定仍看 §4.4 逐条命中与否，与本条改动无关。
    >
    > **本次实跑结果**：用户判词四条已登记；工程影响核对结论为「列表补『流程』列 + 详情屏棒序表**均在本卡变更范围内**，不动契约、不动分堆键、不触发拆卡」；§4.4 止损条件**无一命中**；用户 2026-08-21 明示**放行批 4**。
