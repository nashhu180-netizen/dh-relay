...
> 状态：**未生效候选**。未进 `designInputs[]`，不能当正式计划。确认落盘前只作审核对象。
> 触发：DHR_35 2026-09-01 真实 Claude 闭环 E-3526 / F-3514；用户对话明文「另外开」。
> 上游设计：[design/12](../../design/12-Receipt绑定结果提交与P6真实闭环-契约调整.md)（`DHR-A-26`）P6-RI-A1/A3/A4。**不触发 A-full**：修实现以兑现已冻结语义，不改 Result/Receipt/RPC 合同。
...
Claude 账上最后一条业务事件是 `host_observation_changed`：`herdr_status=idle;agent_get=idle;pane_get=error`。**没有** `E_EXECUTOR_RESULT_MISSING`、**没有** `E_EXECUTOR_HOST_LOST`、**没有** `run_finished`。同一隔离 env 从主控进程再交一次，仍是 `actor-closed`。

`E_LEASE_HELD:actor-closed` 的生产定义：`host.mjs` 里 HostSession actor 已经 `finished`，之后任何 `submitControl`（含写 Result）当场拒绝。
...
**本卡要用夹具钉死的根因（施工时二选一或证明是同一条链）：**

| 编号 | 假说 | 夹具要红的行为 |
|---|---|---|
| H1 | 非终态时 actor/driver 被摘掉或 `finished` 后 `ensureActor` 把已死 actor 交回去 | idle 后、Result 前调用 `submit-executor-result` → 必须 committed Ack，不得 `actor-closed` |
| H2 | `pane_get=error` 使 herdr 等待提前退出且不保留 gate | fake：agent idle + paneGet 失败，提交仍必须通 |
| H3 | 提交打到错误/陈旧 service（隔离根 vs 仓级 endpoint） | 以 DHR_35 同形态的临时根 + 隔离 `DH_RELAY_*` 跑 CLI submit，必须打到写了该 Receipt 的那一届 |

允许路径以 H1 为主（`service.mjs` / `workflow-driver.mjs` / 必要时 `host.mjs`）。H3 若被证成立，只允许动 launcher/CLI 发现，不得改 RPC schema。**不得**顺手改 DHR_35 实录脚本语义，也不得改 Herdr 产品。
...
- **DHR_35**：新增 `blocked-by:DHR_70`。Claude 真实闭环在本卡闭合且用户重新放行前不再跑。Codex E-3525 保留为部分证据，**不得**把「一条产品通了」写成 P6-RI-A4 已满足。
- **验收口径**：
  - **机器证 A**（design/12 P6-RI-A1/A3）：Attempt 非终态 + 已签发 receipt-bound Receipt 时，CLI/RPC `submit-executor-result` 得到 committed Ack 与 `relay.result/v2`（`structured.source=receipt-bound-submission/v1`）；不得 `E_LEASE_HELD:actor-closed`。含「driver 本届 herdr 等待已结束、节点仍非终态」的晚交。
  - **机器证 B**（P6-RI-A3）：未知 Receipt / 已终态冲突 / 同 digest 重投 仍按现役拒绝或幂等，不因修 gate 而变。
  - **机器证 C**：夹具令 `agent_get=idle` 且 `pane_get=error`（对齐 E-3526 观测），提交仍走 A，且不得误写 `E_EXECUTOR_HOST_LOST` 把 Attempt 判死。
...
| D-B34-2 | 晚交时系统应怎样？ | **补交成功**（恢复或保持 gate）。与 design/12 及 Codex E-3525 一致。不把 actor-closed 包装成「请人重开 Attempt」。 |
