...
现状: **P4 阶段闸已解锁**——CM1/2/3/5/6a 全绿 + 主报告落盘（P4 verify `252a131`）+ 用户 2026-08-20 对话放行；**DHR_28 已完成**（verify `0e2dd54`，2026-08-21 用户对话放行）——relay v2 最小协议集已冻结，`relay-core/` 是本仓唯一契约来源。**DHR_29 已于 2026-08-21 经 `DHR-B-15` 拆为三张卡**；DHR_29 与 DHR_51 已完成，DHR_52 进行中，DHR_30 / DHR_31 未开始。
进行到: P5 ▸ **DHR_29 已完成**（2026-08-22 用户对话放行、chat-confirm 代签 verify）；**DHR_51 已完成**（2026-08-22 用户 E11 点选放行、chat-confirm 代签 verify；SHA 回填见本表）；**DHR_52 已于 2026-08-22 经用户对话明示确认开工**，S0~S2 正在落户；DHR_30 / DHR_31 未开始。
下一步: **DHR_52 S3 施工**（RPC 服务端 / 握手 `capability_hash` 比对 fail-closed）——承接项：DHR_51 带出的 F-107 指纹要素评估触发点、as-built §3.6 的 host-lease.json 形态与 fencing 接线。
...
| DHR_52 | 实现 RPC 服务端与握手 fail-closed（含 `capability_hash` 比对） | 标准 · **高危** | 进行中 | DHR_51 | [workspace/DHR_52/](../workspace/DHR_52/brief.md) | | `DHR-B-15` 新建。高危 = 组件接线。用户 2026-08-22 对话确认开工；隔离 worktree 将从包含本户口本的当前本地 master 建，施工 worker=OMP `deepseek-v4-flash`（受 task_plan 精确路径约束）。**开工前 DHR_29 的契约修订批次 1 已收口**，否则基线断言会在施工中途被改动 |
