...
现状: **P4 阶段闸已解锁**——CM1/2/3/5/6a 全绿 + 主报告落盘（P4 verify `252a131`）+ 用户 2026-08-20 对话放行；**DHR_28 已完成**（verify `0e2dd54`，2026-08-21 用户对话放行）——relay v2 最小协议集已冻结，`relay-core/` 是本仓唯一契约来源。**DHR_29 已于 2026-08-21 经 `DHR-B-15` 拆为三张卡**（DHR_29 / DHR_51 / DHR_52），三张均未开始；DHR_30 / DHR_31 未开始
进行到: P5 ▸ **DHR_28 已收口销户；`DHR-B-15` 拆卡已落盘**。交付 = 本仓顶层代码根 [relay-core/](../../../../relay-core/)：7 份冻结协议 + 1 份共享定义模块 + 4 份 v0 形状、23 个 reason code、兼容矩阵、v1 六条缺口处置表、33 份 golden/negative fixture、独立校验器与 11 维度静态审计、三份互不覆盖的基线（fixture manifest / capability baseline / 结构 token 白名单）。首份 as-built 见 [as-built/relay-core.md](../as-built/relay-core.md)
下一步: **DHR_29 开工**（`DHR-B-15` 拆卡已落盘，见 §0.2）。三张卡按 `DHR_29 → DHR_51 → DHR_52` 串行，**每张各需一次用户对话内开工确认**（三张一律标高危）。⚠️ **开工前必读三处**（原文照旧，仅补落点）：①`relay-core/contracts/v1-gap-disposition.md` 末尾的「DHR_30 开工前必读」段（DHR_28 已替 DHR_30 冻掉 `group` / `progress` / `elapsed_seconds` 三个 Read Model 展示字段及其天花板）；②`relay-core/contracts/OPEN-POINTS.md` 的 3 处有意开放点 + **K-1~K-4 四条已知缺口**——K-1（`waiting_human` 无产生者）与 K-2（`attempt_id` 无序后 stale 判定失据）已落 **DHR_29** 验收 6 / 4，K-3（身份 token 内联重复）落 **DHR_29 契约修订批次 1**，K-4（attention `reason` 口径对撞）仍归 P7；③DHR_28 findings 里 `遗留→DHR_29（已确认）` 与 `open` 的条目——**F-064** 落 **DHR_52**、**F-009** 落 **DHR_51**、**F-023** 落 **DHR_52**、**F-042 / F-056 / F-057** 分别落 DHR_29 批次 1 与 DHR_52。
...
| DHR_29 | 实现唯一写者 Store、追加事件账与确定性回放；顺带做契约修订批次 1 | 标准 · **高危** | 未开始 | DHR_28 | <开工时回填 workspace/…> | | `DHR-B-15` 保 ID 缩范围（原含宿主与 RPC，已拆出 DHR_51 / DHR_52）。**高危系主会话判断**：本卡动已冻结契约与三份基线，后果等价于宪章#2 的「数据口径」（改一个字即断握手），宪章五类里无「契约修订」一项 |
