...
进行到: P5 ▸ **DHR_29 已完成**（2026-08-22 用户对话放行、chat-confirm 代签 verify）；**DHR_51 已完成**（2026-08-22 用户 E11 点选放行、chat-confirm 代签 verify；SHA 回填见本表）；DHR_52 / DHR_30 / DHR_31 未开始。
下一步: **开工 DHR_52**（RPC 服务端 / 握手 `capability_hash` 比对 fail-closed）——开工前仍按入口闸单独分流并需用户对话内确认；承接项：DHR_51 带出的 F-107 指纹要素评估触发点、as-built §3.6 的 host-lease.json 形态与 fencing 接线。
