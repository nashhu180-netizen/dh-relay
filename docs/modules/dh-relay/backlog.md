...
### DHR-BL-17 本机 `%TEMP%` 原子重命名间歇性 EPERM，污染整仓测试可信度

- **来源**：DHR_70 收口期取证（工作区 `workspace/DHR_70/` 的 `F-7003` / `F-7004`，证据 `E-7016` / `E-7018`）。用户 2026-09-01 在 E11 尾巴分流里点选「入 backlog」，并明示这条优先——它污染的不是某一张卡，是**整仓测试结果的可信度**。
- **现象**：`Error: EPERM: operation not permitted, rename 'C:\Users\nash\AppData\Local\Temp\<fixture>\.dh-relay\<run>\state.json.<uuid>.tmp' -> '...\state.json'`。Store 落盘走「写临时文件 + 原子重命名」，重命名被间歇性拒绝。
- **影响面（已实测，不是推测）**：
  - `identity-quota.test.mjs` 的 DHR_34 族 **9 条**与 `agent-node.test.mjs` 的 DHR_33 族 **4 条**长期稳定失败，此前一直被记成「等一个永远不来的事件（等满 60s 后 `0 !== 1`）」——60s 只是超时表象，**EPERM 才是成因**（DHR_70 之前的 `F-7003` 描述已按候选-10 标 superseded 留痕）。
  - **不限于那两个套件**：DHR_70 自己的 A2 用例在还原变异后的复跑里也撞过一次，紧接两次复跑零 EPERM。任何写 `state.json` 的测试都可能假红。
  - 同一份代码三轮全量的失败数为 **18 / 15 / 14**（`E-7016`），抖动实锤；这使「与基线对齐」这类判断在本机极难做。
