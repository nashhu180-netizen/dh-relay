...
| E-003 | 机器证（B1 TDD · GREEN） | 同 E-002 命令（实现后） | pass（exit 0；7 例全 OK） | A150：四态正例过 lint、`on:review_ready:nobody`/`on:review-ready:`/空前缀报 A35、跨节点与 R 形态报 A71、§3.5 映射表两行绑定结构断言成立；A145：合法信号被接受、双 token/`ghost`/非判定角色目标/判定角色自写各退 2 报 A145、3 条连续信号（> `rework_max_rounds=2`）均被接受且 attempt 恒 1、`decider=` 并存正例证明 A69 扫描不误命中；A144：`on:review_ready:` launch 在有无信号下均退 2 报 A144、stderr 不含 A70、被拒 launch 不落账（账本逐字节不变） |
| E-004 | 机器证（B1 全量回归） | `cd tools/relay-light && PYTHONDONTWRITEBYTECODE=1 python3 -m unittest test_relay_log -v` | pass（exit 0；186 例全 OK，无既有用例转红） | 全量回归绿——`on:done:` 语义（A70）、状态机迁移表（A60）、attempt/重拉闸（A49/A58/A107）、status schema（A62）等既有合同均未削弱；`test_agent_launch_requires_node_start_and_terminal_agents_are_sealed` 原样保持绿 |
| E-005 | 机器证（B1 全量 runner） | `PYTHONDONTWRITEBYTECODE=1 pwsh -NoProfile -File tools/tests/run-relay-tests.ps1`（cwd=仓根） | pass（exit 0；`RELAY ALL PASS (SKIPPED: 1)`；relay-light 段 186+7 例全 OK） | 仓级全量测试套件绿；`PYTHONDONTWRITEBYTECODE=1` 使 runner 不产生 `__pycache__` |
