...
| code-round2 | code-round1 P0/P1 闭合后，与后四路 Review Batch 并发；fresh 且不复用 round1 | 核 round1 闭合、增量 diff、选择并执行有效 mutation，改坏必须行为红且还原全绿 | devin-sub（general，fresh） | 进行中（Review Batch 2026-09-12 派出） | 进行中 |
| requirement | 同一 Review Batch 独立路径 | 逐字对齐 DevPlan 20 条；无 RLT_01/08/09/10/18 抢跑；术语与职责边界语义 | devin-sub（explore，fresh） | 进行中（Review Batch 2026-09-12 派出） | 进行中 |
| consistency | 同一 Review Batch 独立路径 | design ↔ DevPlan ↔ TOML ↔ SKILL.md/双 adapter ↔ tests 的闭集比对 | devin-sub（explore，fresh） | 进行中（Review Batch 2026-09-12 派出） | 进行中 |
