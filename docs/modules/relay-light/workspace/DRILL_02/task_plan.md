...
单批施工：C1 → checker 小审 PASS → scribe 汇总，随后 R1（lesson + consistency 两路并发）→ F1。每角色完工只追加一行信号到 `progress.md`「信号」节末：
...
各角色 status 取值以当班监工派单逐字为准；写完信号立即停止，不等 `node_closed`，不越位派活。

## 施工共通约束

- 每个证据先登记 `progress.md` 证据账本（E-ID），再从 DONE 的 `evidence=` 引用；禁止悬空描述符。
...
- checker 只读本批 diff/证据写独立 `check.C1.md`；coder 不改 checker 结论或 review.md 复核区。
- 凭据/密钥值永不写入任何工件与账本。
...
7. **收口**：只暂存 `.gitignore` 与本卡 workspace 文件并 commit；progress 记日志/E-ID 后发 `DONE task=DRILL_02 role=coder batch=C1 status=READY_FOR_REVIEW evidence=...,commit=SHA next=monitor`，即停。
...
light Recipe 不设 CONSTRUCTION_DONE 复核闸升级：C1 经 checker 小审、scribe 汇总后进 R1（lesson + consistency 两路，由监工按 relay_plan 拉 reviewer），R1 闭合后 F1 scribe 备料收口。施工者不进入复核，复核者只读不改。
